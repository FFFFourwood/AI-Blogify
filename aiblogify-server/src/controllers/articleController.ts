...
export const getAllArticles = async (req: Request, res: Response): Promise<void> => {
    ...
    logger.info(`Article:Getting all articles`);
    try {
        const articles = await articleService.getAllArticles(Number(page), Number(limit));
        logger.info(`Article:Articles fetched successfully`);
        res.status(200).json(articles);
    } catch (error) {
        logger.error(`Article:Error fetching articles`);
...
export const getArticle = async (req: Request, res: Response) => {
    logger.info(`Article:Getting article by id`);
    try {
        const article = await articleService.getArticleById(req.params.id);
        if (!article) {
            logger.error(`Article:Article not found`);
            return res.status(404).json({ message: 'Article not found' });
        }
        logger.info(`Article:Article fetched successfully`);
        res.json(article);
    } catch (error) {
        logger.error(`Article:Error fetching article`);
...
export const getArticlesByCategory = async (req: Request, res: Response): Promise<void> => {
    ...
    const { page = 1, limit = 10 } = req.query;
    logger.info(`Article:Getting articles by category`);
    try {
        const articles = await articleService.getArticlesByCategory(categoryId, Number(page), Number(limit));
        logger.info(`Article:Articles fetched successfully`);
        res.status(200).json(articles);
    } catch (error) {
        logger.error(`Article:Error fetching articles by category`);
...
export const getArticlesByTag = async (req: Request, res: Response): Promise<void> => {
    ...
    const { page = 1, limit = 10 } = req.query;
    logger.info(`Article:Getting articles by tag`);
    ...
    try {
        ...
        logger.error(`Article:Error fetching articles by tag`);
